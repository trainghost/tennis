...
def assign_courts(players, match_no):
    ...
    if match_no == 1:
        if len(females) >= 5:
            c3 = pick(females, 4)   # 여자복식
            c4 = pick(males, 4)     # 남자복식
            team5 = []
            if females:
                team5.append(females.pop(0))  # 여자 1위
            team5 += pick(males, 3)
            if len(team5) < 4:  # 남자 부족 → 여자 보충
                team5 += pick(females, 4 - len(team5))
            courts[3], courts[4], courts[5] = make_teams(c3), make_teams(c4), make_teams(team5)
